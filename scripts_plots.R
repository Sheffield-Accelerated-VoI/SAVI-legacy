...
makeCEPlanePlot <- function(costs.int, effects.int, lambda, intervention, comparator, cache, ...) {
  ...
  plot(inc_effects, inc_costs,
    pty = "s", cex = 0.4, xlab = "Incremental effects", ylab = "Incremental costs",
...
makeCeacPlot <- function(ceac.int, lambda.int, names.int, ...) {
  ## makes the CEAC plot
  plot(ceac.int$l.seq, ceac.int$p[, 1],
    type = "l", ylim = c(0, 1),
...
makeNbDensity <- function(costs.int, effects.int, lambda) {
  ...
  ymax <- max(ymax)
  plot(density(nb[, 1]),
    type = "l", col = 1, xlim = c(xmin, xmax), ylim = c(0, ymax),
...
makeInbOptDens <- function(costs.int, effects.int, lambda) {
  ...
  ymax <- max(ymax)
  plot(density(inbOpt[, 1]),
    type = "l", col = 1, xlim = c(xmin, xmax),
    ylim = c(0, ymax), xlab = "INB vs. Optimal Strategy",
    main = paste(
      "Incremental Net Benefit Density\nOptimal Strategy is",
...
makeEvpiPopPlot <- function(costs.int, effects.int, costscale = TRUE, lambda, prevalence,
  ...
  plot(l.seq, p,
    type = "l", main = paste("Overall EVPI per annual prevalence ", measure),
    xlab = "Threshold willingness to pay", ylab = paste(
      "Annual population EVPI ",
      measure
    ), col = "blue"
  )
  abline(v = lambda, lty = 2)
...
makeEvpiHorizonPlot <- function(costs.int, effects.int, costscale = TRUE, lambda, prevalence,
  ...
  plot(l.seq, p,
    type = "l", main = paste("Overall EVPI over decision relevance ", measure),
    xlab = "Threshold willingness to pay", ylab = paste(
      "Annual population EVPI ",
      measure
    ), col = "blue"
...
make4wayEvpiPlot <- function(costs.int, effects.int, lambda, prevalence, horizon, measure1,
  ...
  makeEvpiPopPlot(costs.int, effects.int,
    costscale = TRUE,
  ...
  makeEvpiHorizonPlot(costs.int, effects.int,
    costscale = TRUE,
...
makePSUBplot <- function(costs.int, effects.int, lambda, annualPrev, benUnit, beside) {
  ...
  legend("topright",
    inset = c(-0.21, 0), xpd = TRUE, col = rev(psubCols), y.intersp = 2, pch = 15,
  ...
  mtext(
    side = 1, paste("Population-level burdens: ", annualPrev, " people affected by decision p.a. in jurisdiction", sep = ""),
    line = 4, col = "black", adj = 0
  )
  mtext(side = 1, paste("Population PUB p.a. (EVPI) = \u00A3", format(PUBmillion, digits = 2, nsmall = 2), "m (",
    format(PUBQual, digits = 2, nsmall = 2), " QALYs worth of uncertainty p.a.)",
    sep = ""
  ), line = 6, col = "#4F81BD", adj = 0)
  mtext(
    side = 1, expression(underline("Population Payer Strategy-Specific Risk Burdens for each strategy:")),
    line = 8, col = "red", adj = 0
  )

  for (selectOption in 1:length(psbsPopQaly)) {
    mtext(
      side = 1, paste("Population PSB = \u00A3", format(psbsPop[selectOption], digits = 2, nsmall = 2), "m (",
        format(psbsPopQaly[selectOption], digits = 2, nsmall = 2), " QALYs) for ", names(costs.int)[selectOption], " ",
        sep = ""
      ),
