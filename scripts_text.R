...
aboveBelow <- function(costs, bens, int, comp, lambda, cache) {
  ...
  aboveBelow <- if ((mean(costs[, int] - costs[, comp]) / mean(bens[, int] - bens[, comp])) > lambda) {
    "above"
  } else {
    "below"
  }
...
wouldNot <- function(costs, bens, int, comp, lambda, cache) {
  ...
  wouldNot <- if ((mean(costs[, int] - costs[, comp]) / mean(bens[, int] - bens[, comp])) > lambda) {
    "would not"
  } else {
    "would"
  }
