  ...
  function(input, output, session) {
    ...
    output$textCheckTabParams <- renderText({
      ...
      if (inherits(params, "try-error")) {
        return("There is something wrong with the parameters csv file. Probably formatting.
      ...
      if (!prod(unlist(c(lapply(params, function(x) {
    ...
    output$textCheckTabCosts <- renderText({
      ...
      if (inherits(costs, "try-error")) {
        return("There is something wrong with the costs csv file. Probably formatting.
                                      Please check and remove all formatting")
      }

      if (sum(is.na(costs)) > 0) {
        return("There are missing values - please check data and reload")
      }

      if (NCOL(costs) == 1) {
        return("There must be at least two decision options.
      ...
      if (!prod(unlist(c(lapply(costs, function(x) {
        class(x) == "numeric" | class(x) == "integer"
      }))))) {
        return("Not all columns are numeric - please check data and reload")
      }
    ...
    output$textCheckTabEffects <- renderText({
      ...
      if (inherits(effects, "try-error")) {
        return("There is something wrong with the effects csv file. Probably formatting.
                                         Please check and remove all formatting")
      }
      ...
      if (!prod(unlist(c(lapply(effects, function(x) {
        class(x) == "numeric" | class(x) == "integer"
      }))))) {
        return("Not all columns are numeric - please check data and reload")
      }
    ...
    output$textCheckTab <- renderText({
      ...
      if (inherits(params, "try-error") | inherits(costs, "try-error") | inherits(effects, "try-error")) {
        return(NULL)
      }
    ...
    output$textParamsTable <- renderText({
      ...
      paste(
        "<em>There are ", NROW(cache$params), " rows and ", NCOL(cache$params),
    ...
    output$textCostsTable <- renderText({
      ...
      # dummy <- input$indSim # ensure update with ind sim box tick
      paste(
        "<em>There are ", NROW(cache$costs), " rows and ", NCOL(cache$costs),
    ...
    output$textEffectsTable <- renderText({
      ...
      # dummy <- input$indSim # ensure update with ind sim box tick
      paste(
        "<em>There are ", NROW(cache$effects), " rows and ", NCOL(cache$effects),
    ...
    output$textCEplane1 <- renderText({
      ...
      cache$pCostsavingVal <- pCostsaving(
        cache$costs, input$decisionOptionCE1,
        input$decisionOptionCE0, cache
      )
      cache$incValueCosts <- incValue(
        cache$costs, input$decisionOptionCE1,
        input$decisionOptionCE0, cache
      )

      cache$confIntCE025costs <- confIntCE(
        cache$costs, input$decisionOptionCE1,
        input$decisionOptionCE0, 0.025, cache
      )

      cache$confIntCE975costs <- confIntCE(
        cache$costs, input$decisionOptionCE1,
        input$decisionOptionCE0, 0.975, cache
      )

      cache$moreLessCosts <- moreLess(
        cache$costs, input$decisionOptionCE1,
        input$decisionOptionCE0, cache
      )
    ...
    output$textCEplane2 <- renderText({
      ...
      cache$incValueEffects <- incValue(
        cache$effects, input$decisionOptionCE1,
        input$decisionOptionCE0, cache
      )

      cache$confIntCE025effects <- confIntCE(
        cache$effects, input$decisionOptionCE1,
        input$decisionOptionCE0, 0.025, cache
      )

      cache$confIntCE975effects <- confIntCE(
        cache$effects, input$decisionOptionCE1,
        input$decisionOptionCE0, 0.975, cache
      )

      cache$pMorebenVal <- pMoreben(
        cache$effects, input$decisionOptionCE1,
        input$decisionOptionCE0, cache
      )

      cache$moreLessEffects <- moreLess(
        cache$effects, input$decisionOptionCE1,
        input$decisionOptionCE0, cache
      )
    ...
    output$textCEplane3 <- renderText({
      ...
      cache$pCEVal <- pCE(
        input$decisionOptionCE1, input$decisionOptionCE0,
    ...
    output$tableCEplane <- renderTable(
      {
        ...
        cache$lambdaOverall <- input$lambdaOverall
        rownames(tableCEplane) <- c(
          paste("Threshold (", input$currency, " per ", input$unitBens, ")", sep = ""),
          "Comparator",
          "Number of PSA runs",
    ...
    output$textCEAC1 <- renderText({
      ...
      cache$bestCEVal <- bestCE(
        cache$costs, cache$effects,
        input$lambdaOverall, cache$nInt
      )

      cache$highestpCE <- highestCE(
        cache$costs, cache$effects,
        input$lambdaOverall
      )
      ...
      paste("This graph shows the cost-effectiveness acceptability curve for the
        ...
        ". More details on how to interpret CEACs are available from the literature.",
        sep = ""
      )
    })
    ...
    output$textNB3 <- renderText({
      ...
      cache$bestnetBenVal <- bestnetBen(
        cache$costs,
        cache$effects, input$lambdaOverall, cache$nInt
      )

      cache$netBencostsVal <- netBencosts(
        cache$costs, cache$effects,
        input$lambdaOverall, cache$nInt
      )

      cache$netBeneffectsVal <- netBeneffects(
        cache$costs, cache$effects,
        input$lambdaOverall, cache$nInt
      )
    ...
    output$tableNetBenefit <- renderTable(
      {
        ...
        cache$lambdaOverall <- input$lambdaOverall
        rownames(tableNetBenefit) <- c(
          paste("Mean", input$effectDef),
    ...
    output$textEVPI1 <- renderText({
      ...
      paste("The overall EVPI per person affected by the decision is estimated to be ",
        input$currency, format(calcEvpi(
          cache$costs,
          cache$effects, input$lambdaOverall
        ), digits = 4, nsmall = 2), ".  This is equivalent to ",
        format(calcEvpi(cache$costs, cache$effects, input$lambdaOverall) / input$lambdaOverall,
          digits = 4, nsmall = 1
        ), " ", input$unitBens,
        "s per person on the health effects scale.",
        sep = ""
      )
    })
    ...
    output$tableEVPI <- renderTable(
      {
        ...
        rownames(tableEVPI) <- c(
          "Per Person Affected by the Decision",
          paste(
            "Per Year in", input$jurisdiction, "Assuming",
        ...
        cache$lambdaOverall <- input$lambdaOverall
        evpiVector <- c(
          overallEvpi, overallEvpi * input$annualPrev, overallEvpi * input$annualPrev * 5,
    ...
    output$plots3 <- renderPlot({
      ...
      cache$lambdaOverall <- input$lambdaOverall
      makeEvpiPlot(cache$costs, cache$effects,
        lambda = input$lambdaOverall,
        main = input$main3,
    ...
    output$plots6 <- renderPlot({
      ...
      make4wayEvpiPlot(cache$costs, cache$effects,
        lambda = input$lambdaOverall,
    ...
    output$tableEVPPI <- renderTable({
        ...
        colnames(tableEVPPI) <- c(
          paste("Per Person EVPPI (", input$currency, ")", sep = ""),
          "Standard Error", "Indexed to Overall EVPI = 1.00",
          paste("EVPPI for ", input$jurisdiction,
            " Per Year (", input$currency, ")",
            sep = ""
          ),
          paste("EVPPI for ", input$jurisdiction, " over ",
            input$horizon, " years (", input$currency, ")",
            sep = ""
          )
        )
    ...
    output$plot7 <- renderPlot(
      ...
      height = function() max(150, nrow(cache$pEVPI) * 30)
    )
    ...
    output$selectedTable <- renderTable(
      {
        ...
        if (!isolate(valuesImportedFLAG(cache, input))) {
          return(NULL)
        }
        if (dummy == 0) {
          return(NULL)
        ...
        if (is.null(subsetEvpiValues)) {
          subsetEvpiValues <- t(sapply(
            setStore[1:counterAdd], calc,
            input$lambdaOverall, cache, session
          ))
        } else {
          newEvpiValue <- t(sapply(
            setStore[(NROW(subsetEvpiValues) + 1):counterAdd],
        ...
        if (!is.null(contents)) {
          ...
          colnames(contents) <- c(
            ...
            paste("EVPPI for ", cache$jurisdiction,
              " Per Year (", cache$currency, ")",
              sep = ""
            ),
            paste("EVPPI for ", cache$jurisdiction,
              " over ", cache$horizon, " years (", cache$currency, ")",
              sep = ""
            )
          )
        }
    ...
    output$plotsPSUBstacked <- renderPlot({
      ...
      makePSUBplot(cache$costs, cache$effects,
        lambda = input$lambdaOverall, input$annualPrev, benUnit = input$unitBens,
        beside = FALSE
      )
    })
