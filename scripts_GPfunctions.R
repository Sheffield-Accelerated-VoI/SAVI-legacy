...
estimate.hyperparameters <- function(NB, inputs, session) {
  ...
  progress1$set(
    message = "Estimating GP hyperparameters",
    detail = "Please wait..."
  )
  ...
  for (d in 2:D) {
    ...
    repeat {
      ...
      log.hyperparameters <- optim(initial.values,
        ...
        method = "Nelder-Mead",
        control = list(fnscale = -1, maxit = 10000, trace = 0)
      )$par
...
gpFunc <- function(NB, sets, s = 1000, cache, session) {
  ...
  if (sum(constantParams) == length(sets)) {
    return(list(EVPI = 0, SE = 0))
  } # if all regressors are constant
  ...
  m <- min(30 * p, 250)
  m <- min(nrow(NB), m)
  setForHyperparamEst <- 1:m 
  hyperparameters <- estimate.hyperparameters(
  ...
  progress1 <- shiny::Progress$new(session, min = 1, max = D)
  progress1$set(
    message = "Calculating conditional expected net benefits",
    detail = "Please wait..."
  )
  progress1$set(value = 1)
  ...
  progress2$set(
    message = "Calculating Standard Error",
    detail = "Please wait..."
  )
  progress2$set(value = 1)

  set.seed(round(abs(NB[1, 1]) + 2))
  for (d in 2:D) {
