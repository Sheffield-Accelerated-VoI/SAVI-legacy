...
valuesImportedFLAG <- function(cache, input) {
  ...
  if (is.null(cache$params) |
      ...
      inherits(cache$uploadedEffects, "try-error")) {
    return(FALSE)
  } else {
    return(TRUE)
  }
...
calSubsetEvpi <- function(sets, lambda, cache, session) {
  numParams <- length(sets)

  regressionFunction <- if (numParams > 4) "gpFunc" else "gamFunc"

  f <- formulaGenerator(sets)
  nb <- cache$effects * lambda - cache$costs
  inb <- nb - nb[, 1]
...
word_document_local <- function(fig_width = 5, fig_height = 4, fig_caption = FALSE,
  ...
  knitr <- knitr_options(opts_chunk = list(
    dev = "png", dpi = 96, fig.width = fig_width,
  ...
  if (!is.null(reference_docx) && !identical(
    reference_docx,
    "default"
  )) {
  ...
  output_format(knitr = knitr, pandoc = rmarkdown::pandoc_options(
    to = "docx",
